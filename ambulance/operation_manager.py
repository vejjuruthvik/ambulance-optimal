...
class RouteManager:
    ...
    def get_nodes(self, start_loc_coords, point_loc, mode='drive'):
        graph = ox.graph_from_point(start_loc_coords, network_type=mode)
        orig_node = ox.get_nearest_node(graph, start_loc_coords, method='euclidean')
        dest_node = ox.get_nearest_node(graph, point_loc, method='euclidean')
        
        map_dict = {'graph':graph, 'origin':orig_node, 'destination':dest_node}
...
class Route:
    ...
    def __init__(self, start=0, end=0, Size=0):
        self.route = []
        self.distance = 0
        self.fitness = 0
    ...
    def generateMap(self, map, optimizer='time'):
        shortest_route = nx.shortest_path(map['graph'], map['origin'], map['destination'], weight=optimizer)
        shortest_route_map = ox.plot_route_folium(map['graph'], shortest_route, tiles='OpenStreetMap')
